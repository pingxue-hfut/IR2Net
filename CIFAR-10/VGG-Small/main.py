...
def main():
    ...
    logging.basicConfig(level=logging.INFO, filename='log/' + ''.join(args.arch) + '.log', format='%(message)s')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best and epoch > 0 and epoch % args.save_every == 0:
            save_checkpoint({
                'epoch': epoch + 1,
        ...
        log_message = str(epoch) + '\t' + '{:.3f}'.format(train_loss) + '\t' + '{:.3f}'.format(val_loss) + '\t' + '{:.3f}'.format(train_acc) + '\t' + '{:.3f}'.format(prec1) + '\t'
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        output1, features = model(input_var)
        ae = attention_erase(features, input_var)
        output2, _ = model(ae)
