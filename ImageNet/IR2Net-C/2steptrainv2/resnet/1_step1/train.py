...
parser.add_argument('--data', metavar='DIR', default='C:/Users/wsco/Desktop/imagenet', help='path to dataset')
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.epochs, eta_min=0, last_epoch=159)
    start_epoch = 0
    best_top1_acc = 0
...
def train(epoch, train_loader, model_student, model_teacher, criterion, optimizer, scheduler):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute outputy
        output1, feature = model_student(images)
        erase_img = attention_erase(feature, images)
        output2, _ = model_student(erase_img)
        logits_teacher = model_teacher(images)
        loss1 = criterion(output1, logits_teacher)
        loss2 = criterion(output2, logits_teacher)
        loss = (loss1 + loss2) / 2
...
def attention_erase(attention_maps, input_image, lamda=0.15):
    ...
    input = input_image
    attention_maps = at(attention_maps)
